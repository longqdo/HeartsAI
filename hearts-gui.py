...
def setNumPlayers(num, frame_old, frame_new):
    ...
    global players_list_index
    numplayers = num
    players_list = players_init(numplayers)
    players_list = init(players_list, numplayers)
    players_list_wrap = np.concatenate((players_list, players_list))
    first = goesFirst(players_list)
    players_list_index = np.where(players_list == first)[0][0]
...
class StartPage(tk.Frame):
    def __init__(self, master):
        ...
        tk.Label(self, text="How many players would you like in the game?").pack(side="top", fill="x", pady=10)
        tk.Button(self, text="3 Players",
                  command=lambda: setNumPlayers(3, master, PageOne)).pack()
        tk.Button(self, text="4 players",
                  command=lambda: setNumPlayers(4, master, PageOne)).pack()
...
class PagePlay(tk.Frame):
    def __init__(self, master):
        ...
        #tk.Label(self, text="Click on the card you would like to play " + curr_player.name).grid(row = 2, column = 0)
